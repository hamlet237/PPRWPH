...
class PPRWPH_Functions_Post {
	 ...
	 * @since    1.0.0
	 */
	public static function insert_post($title, $content, $excerpt, $name, $type, $status, $author = 1, $parent = 0, $cats = [], $tags = [], $postmeta = [], $overwrite_id = true) {
    $post_values = [
      ...
      'ping_status' => 'closed',
      'menu_order' => self::get_last_order($type),
    ];
  ...
  public static function get_last_order($type) {
    $last_id = get_posts(['fields' => 'ids', 'numberposts' => -1, 'post_type' => 'pprwph_white', 'post_status' => ['any'], 'orderby' => 'menu_order', 'order' => 'DESC', ]);

    return (intval(get_post($last_id[0])->menu_order) + 1);
  }
}